...
import com.tasktracker.app.model.TaskRelation;
...
public class TaskParametersView
{
    ...
    public TaskParametersView(Dimension dimension)
    {
                ...
                for (Map.Entry<Parameter, JTextComponent> entry : parameters.entrySet())
                {
                    ...
                    if (textComponent.getBackground().equals(Color.GREEN))
                    {
                        try
                        {
                            ...
                            switch(parameter)
                            {
                                case NAME: curTask.setName(newValue);
                                    //case TASK_ID: curTask.setName(textComponent.getText());
                                case PARENT_ID: curTask.setParentID(Integer.valueOf(newValue));
                                case STATUS: curTask.setStatus(Task.Status.valueOf(newValue));
                                case PRIORITY: curTask.setPriority(Integer.valueOf(newValue));
                                case START_DATE: curTask.setStartDate(DateFormat.getInstance().parse(newValue));
                                case END_DATE: curTask.setEndDate(DateFormat.getInstance().parse(newValue));
                                case DESCRIPTION: curTask.setDescription(newValue);
                            }
                        }
    ...
    private void initRelationsPanel()
    {
        ...
        Collection<User> users = ModelContainer.getInstance().getAllUsers();
        for (User user : users)
        {
